...
import templateCreators.DeckCoreTemplateCreator;

public class DeckCoreWindow extends Stage implements DialogClass {
    ...
    private TextField monstersTextField;
    private TextField spellsTrapsTextField;
    private TextField extraDeckTextField;
    ...
    private void saveData() {
        ...
        DeckCoreTemplateCreator creator = new DeckCoreTemplateCreator(monsterData, spellTrapData, extraDeckData, relatedCards);
	...
	private void createTable() {
     ...
     // add rows for monsters
        ...
        for (int i = 2; i < numMonsters+2; i++) {
            ...
            // Update the monsterTypeField values for the current and subsequent rows
            final int index = i;  // Create a final variable for the index
            monsterTypeField.setOnAction(event -> {
                String newValue = monsterTypeField.getValue();
                updateMonsterTypeFields(index, newValue);
            });
        ...
        for (int i = 2; i < numSpellsTraps+2; i++) {
            ...
            // Update the monsterTypeField values for the current and subsequent rows
            final int index = i;  // Create a final variable for the index
            spellTrapTypeField.setOnAction(event -> {
                String newValue = spellTrapTypeField.getValue();
                updateSpellTrapTypeFields(index, newValue);
            });
        ...
        for (int i = 2; i < numExtraDeck+2; i++) {
            ...
            // Update the monsterTypeField values for the current and subsequent rows
            final int index = i;  // Create a final variable for the index
            extraDeckTypeField.setOnAction(event -> {
                String newValue = extraDeckTypeField.getValue();
                updateExtraTypeFields(index, newValue);
            });
	...
	private void updateMonsterTypeFields(int startIndex, String newValue) {
        // Iterate over the monsterTypeFields starting from startIndex and update their values
        for (int i = startIndex + 1; i < numMonsters + 2; i++) {
            ChoiceBox<String> monsterTypeField = (ChoiceBox<String>) gridBox.lookup("#monsterTypeField" + i);
            monsterTypeField.setValue(newValue);
        }
    }
	
	private void updateSpellTrapTypeFields(int startIndex, String newValue) {
        // Iterate over the monsterTypeFields starting from startIndex and update their values
        for (int i = startIndex + 1; i < numSpellsTraps + 2; i++) {
            ChoiceBox<String> spellTrapField = (ChoiceBox<String>) gridBox.lookup("#spellTrapTypeField" + i);
            spellTrapField.setValue(newValue);
        }
    }
	
	private void updateExtraTypeFields(int startIndex, String newValue) {
        // Iterate over the monsterTypeFields starting from startIndex and update their values
        for (int i = startIndex + 1; i < numExtraDeck + 2; i++) {
            ChoiceBox<String> extraTypeField = (ChoiceBox<String>) gridBox.lookup("#extraDeckTypeField" + i);
            extraTypeField.setValue(newValue);
        }
    }
}